...
bool read_bin(char *in, char *outf) {
	uint16_t crc;
	FILE *fp, *out;
	int i,j;
	...
	for (i = 0; i<header.numatoms; i++) {
		...
		if (atom.type==ATOM_VENDOR_TYPE) {
			...
			if (!fread(vinf.vstr, vinf.vslen, 1, fp))
				goto err;
			if (!fread(vinf.pstr, vinf.pslen, 1, fp))
				goto err;
			...
			if (!fread(&crc, CRC_SIZE, 1, fp))
				goto err;
			
		} else if (atom.type==ATOM_GPIO_TYPE) {
			//decode GPIO map
			if (!fread(&gpiomap, GPIO_SIZE, 1, fp))
				goto err;
			...
			if (!fread(data, atom.dlen-CRC_SIZE, 1, fp))
				goto err;
